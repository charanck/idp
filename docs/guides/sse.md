...
Only `inapp` sends ever publish an SSE event — `email` and `sms` are fire-and-forget with no live
push, since there's no "the recipient is watching a stream" concept for those channels. Poll
[In-app inbox](inapp-inbox.md) or your own delivery records if you need status for those.
