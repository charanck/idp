...
func TestNotificationAPIKeyAuthMiddleware_MissingHeaderReturns401(t *testing.T) {
	mw := apihttp.NewNotificationAPIKeyAuthMiddleware(&fakeNotificationAuthenticator{}, &fakeNotificationRateLimiter{}, 60, 100)
	called := false
...
func TestNotificationAPIKeyAuthMiddleware_RateLimitedReturns429WithRetryAfter(t *testing.T) {
	mw := apihttp.NewNotificationAPIKeyAuthMiddleware(&fakeNotificationAuthenticator{}, &fakeNotificationRateLimiter{limited: true}, 60, 100)
	called := false
	handler := mw.Middleware()(func(c echo.Context) error {
		called = true
		return nil
...
func TestNotificationAPIKeyAuthMiddleware_InvalidKeyReturns401(t *testing.T) {
	mw := apihttp.NewNotificationAPIKeyAuthMiddleware(&fakeNotificationAuthenticator{subject: ""}, &fakeNotificationRateLimiter{}, 60, 100)
...
func TestNotificationAPIKeyAuthMiddleware_ValidKeySetsSubjectAndCallsNext(t *testing.T) {
	mw := apihttp.NewNotificationAPIKeyAuthMiddleware(&fakeNotificationAuthenticator{subject: "billing-service"}, &fakeNotificationRateLimiter{}, 60, 100)

	var gotSubject string
	handler := mw.Middleware()(func(c echo.Context) error {
		gotSubject = apihttp.SubjectFromContext(c)
		return nil
	})
