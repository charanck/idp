...
func setupConfigService(t *testing.T) (*gorm.DB, *config.ConfigService, *crypto.EncryptionService) {
	t.Helper()
...
func TestUpsertConfig_CreatesApplicationAndEnvironmentOnFirstWrite(t *testing.T) {
	gdb, svc, _ := setupConfigService(t)
	...
	var appCount int64
	gdb.Model(&config.Application{}).Where("name = ?", "payments").Count(&appCount)
	if appCount != 1 {
		t.Fatalf("expected application to be created")
	}
	var envCount int64
	gdb.Model(&config.Environment{}).Joins("JOIN applications ON applications.id = environments.application_id").
		Where("applications.name = ? AND environments.name = ?", "payments", "prod").Count(&envCount)
	if envCount != 1 {
		t.Fatalf("expected environment to be created")
	}
}
...
func TestUpsertConfig_UpdatesExistingEntryInsteadOfDuplicating(t *testing.T) {
	...
	var count int64
	gdb.Model(&config.ConfigEntry{}).
		Joins("JOIN applications ON applications.id = config_entries.application_id").
		Where("applications.name = ? AND config_entries.key = ?", "payments", "API_URL").
		Count(&count)
	if count != 1 {
...
func TestUpsertConfig_ReusesScopeAcrossMultipleKeys(t *testing.T) {
	gdb, svc, _ := setupConfigService(t)
	ctx := context.Background()
	...
	gdb.Model(&config.Application{}).Where("name = ?", "payments").Count(&appCount)
	if appCount != 1 {
		t.Fatalf("expected 1 application, got %d", appCount)
	}
}
...
func TestDecryptConfigValueOrOriginal_ReturnsPlaintextForLegacyRow(t *testing.T) {
	...
	app := &config.Application{Name: "legacy-app"}
	if err := gdb.Create(app).Error; err != nil {
		t.Fatalf("create app: %v", err)
	}
	env := &config.Environment{ApplicationID: app.ID, Name: "prod"}
	if err := gdb.Create(env).Error; err != nil {
		t.Fatalf("create env: %v", err)
	}
	legacyEntry := &config.ConfigEntry{ApplicationID: app.ID, EnvironmentID: env.ID, Key: "LEGACY", Value: "plaintext-value"}
	if err := gdb.Create(legacyEntry).Error; err != nil {
...
func TestDeleteConfig_DeletesExistingConfig(t *testing.T) {
	gdb, svc, _ := setupConfigService(t)
	ctx := context.Background()
	entry, err := svc.UpsertConfig(ctx, "payments", "prod", "KEY_A", "a", config.UpsertOptions{})
	if err != nil {
		t.Fatalf("UpsertConfig: %v", err)
	}

	ok, err := svc.DeleteConfig(ctx, entry.ID.String())
	if err != nil {
	...
	var count int64
	gdb.Model(&config.ConfigEntry{}).Where("id = ?", entry.ID).Count(&count)
	if count != 0 {
...
func TestDeleteConfig_ReturnsFalseForUnknownID(t *testing.T) {
	_, svc, _ := setupConfigService(t)
	ok, err := svc.DeleteConfig(context.Background(), uuid.New().String())
	if err != nil {
		t.Fatalf("DeleteConfig: %v", err)
	}
	if ok {
...
func TestDeleteConfig_ReturnsFalseForMalformedID(t *testing.T) {
	_, svc, _ := setupConfigService(t)
	ok, err := svc.DeleteConfig(context.Background(), "not-a-uuid")
	if err != nil {
		t.Fatalf("DeleteConfig: %v", err)
	}
	if ok {
...
func TestConfigHistory_DeletingConfigDeletesItsHistory(t *testing.T) {
	...
	var count int64
	gdb.Model(&config.ConfigEntryVersion{}).Where("config_entry_id = ?", entry.ID).Count(&count)
	if count != 0 {
		t.Fatal("expected version rows to be gone")
	}
}
...
func TestRollbackConfig_RestoresPriorValueAsNewVersion(t *testing.T) {
	...
	decrypted, _ := svc.DecryptConfigValue(rolledBack)
	if decrypted != "https://v1.example.com" {
		t.Fatalf("decrypted = %q", decrypted)
	}
	...
	if len(history) != 3 || history[0].Action != config.ActionRollback || history[1].Action != config.ActionUpdate || history[2].Action != config.ActionCreate {
		t.Fatalf("unexpected history: %+v", history)
	}
	if history[0].Version != 3 {
		t.Fatalf("version = %d", history[0].Version)
	}
	if history[0].ChangedBy == nil || *history[0].ChangedBy != "admin@example.com" {
		t.Fatalf("changed_by = %v", history[0].ChangedBy)
	}
...
func TestRollbackConfig_ReturnsNilForUnknownVersion(t *testing.T) {
	...
	got, err := svc.RollbackConfig(ctx, entry.ID.String(), 99, "")
	if err != nil {
		t.Fatalf("RollbackConfig: %v", err)
	}
	if got != nil {
...
func TestListConfigsForClient_SkipsEntriesNotDecryptableUnderMasterKey(t *testing.T) {
	...
	env := &config.Environment{ApplicationID: app.ID, Name: "prod"}
	gdb.Create(env)
	gdb.Create(&config.ConfigEntry{ApplicationID: app.ID, EnvironmentID: env.ID, Key: "GOOD", Value: "not-valid-fernet-data"})
	clientKey, _ := crypto.GenerateKey()
...
func TestListConfigsForClient_IsCachedUntilScopeInvalidated(t *testing.T) {
	gdb, svc, _ := setupConfigService(t)
	ctx := context.Background()
	svc.UpsertConfig(ctx, "payments", "prod", "KEY_A", "a", config.UpsertOptions{})
	clientKey, _ := crypto.GenerateKey()
	...
	if err != nil {
		t.Fatalf("ListConfigsForClient 1: %v", err)
	}

	// Mutate the row directly, bypassing UpsertConfig's cache invalidation.
	gdb.Model(&config.ConfigEntry{}).Where("key = ?", "KEY_A").Update("value", "corrupted-and-should-not-be-seen")

	second, err := svc.ListConfigsForClient(ctx, "payments", "prod", clientKey)
	if err != nil {
		t.Fatalf("ListConfigsForClient 2: %v", err)
	}
...
func TestListConfigsForClient_CacheInvalidatedAfterUpsert(t *testing.T) {
	_, svc, _ := setupConfigService(t)
	ctx := context.Background()
	svc.UpsertConfig(ctx, "payments", "prod", "KEY_A", "a", config.UpsertOptions{})
	clientKey, _ := crypto.GenerateKey()
	svc.ListConfigsForClient(ctx, "payments", "prod", clientKey)

	svc.UpsertConfig(ctx, "payments", "prod", "KEY_A", "a-updated", config.UpsertOptions{})
	results, err := svc.ListConfigsForClient(ctx, "payments", "prod", clientKey)
	if err != nil {
		t.Fatalf("ListConfigsForClient: %v", err)
	}
	decrypted, err := crypto.DecryptValue(results[0].Value, clientKey)
	if err != nil {
		t.Fatalf("DecryptValue: %v", err)
	}
	if decrypted != "a-updated" {
		t.Fatalf("decrypted = %q", decrypted)
	}
}
