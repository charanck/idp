...
func TestAPIKeyAuthMiddleware_MissingHeaderReturns401(t *testing.T) {
	mw := api.NewAPIKeyAuthMiddleware(&fakeAPIKeyAuthenticator{}, &fakeRateLimiter{}, 60, 100)
	called := false
...
func TestAPIKeyAuthMiddleware_RateLimitedReturns429WithRetryAfter(t *testing.T) {
	mw := api.NewAPIKeyAuthMiddleware(&fakeAPIKeyAuthenticator{}, &fakeRateLimiter{limited: true}, 60, 100)
	called := false
	handler := mw.Middleware()(func(c echo.Context) error {
		called = true
		return nil
...
func TestAPIKeyAuthMiddleware_InvalidKeyReturns401(t *testing.T) {
	mw := api.NewAPIKeyAuthMiddleware(&fakeAPIKeyAuthenticator{client: nil}, &fakeRateLimiter{}, 60, 100)
...
func TestAPIKeyAuthMiddleware_ValidKeySetsClientAndCallsNext(t *testing.T) {
	...
	mw := api.NewAPIKeyAuthMiddleware(&fakeAPIKeyAuthenticator{client: client}, &fakeRateLimiter{}, 60, 100)

	var gotClient *auth.ServiceClient
	handler := mw.Middleware()(func(c echo.Context) error {
		gotClient = api.ServiceClientFromContext(c)
		return nil
	})
