...
func newAuthHandlerFixture(authStore *fakeAuthStore, limiter fakeRateLimiter) (*fakeActivityRecorder, *webui.AuthHandler) {
	activity := &fakeActivityRecorder{}
	h := webui.NewAuthHandler(authStore, fakeOAuthActiveLister{}, limiter, activity, 5, 60)
	return activity, h
}
