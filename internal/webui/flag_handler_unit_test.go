...
func newFlagHandlerFixture() (*fakeFlagStore, *fakeApplicationStore, *fakeEnvironmentStore, *fakeActivityRecorder, *webui.FlagHandler) {
	flags := newFakeFlagStore()
	apps := newFakeApplicationStore()
	envs := newFakeEnvironmentStore()
	activity := &fakeActivityRecorder{}
	h := webui.NewFlagHandler(flags, apps, envs, activity)
